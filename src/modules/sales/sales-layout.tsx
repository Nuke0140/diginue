...
const navItems: { id: SalesPage; label: string; icon: React.ElementType; badge?: string }[] = [
...
function PageContent() {
  const { currentPage } = useSalesStore();
        ...
        <header className={cn(
          ...
          <div className="flex items-center gap-2">
            <div className={cn(
              ...
              <input type="text" placeholder="Search leads, deals... (⌘K)"
              ...
              <kbd className={cn('hidden lg:inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-mono',
              ...
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="relative h-8 w-8 rounded-lg">
                  ...
                  <span className={cn('absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center',
                    isDark ? 'bg-white text-black' : 'bg-black text-white')}>7</span>
                </Button>
              </TooltipTrigger>
          ...
          <AnimatePresence>
            {sidebarOpen && (
              ...
              >
                      ...
                      <button key={item.id} onClick={() => { navigateTo(item.id); if (isMobile) setSidebarOpen(false); }}
                        ...
                        {item.badge && (
                          <Badge variant="secondary"
                            className="ml-auto text-[9px] px-1.5 py-0 bg-red-500/15 text-red-400 border-0">
                            {item.badge}
                          </Badge>
                        )}
                      </button>
                ...
                {/* Sidebar Footer - Revenue Snapshot */}
                <div className={cn('p-3 border-t', isDark ? 'border-white/[0.06]' : 'border-black/[0.06]')}>
                  <div className={cn('rounded-xl p-3 border',
                    ...
                    <p className={cn('text-xl font-bold', isDark ? 'text-white' : 'text-black')}>$2.46M</p>
                    <p className={cn('text-[11px] mt-0.5', isDark ? 'text-white/40' : 'text-black/40')}>
                      8 active deals · 3 closing this week
                    </p>
                  </div>
                </div>
              </motion.aside>
